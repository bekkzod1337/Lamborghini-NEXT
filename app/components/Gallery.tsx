      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-8 px-6 md:px-12">
        {images.map((src, idx) => (
          <motion.div
            ...
            className="relative group overflow-hidden rounded-2xl border border-yellow-500/20 shadow-xl backdrop-blur-md"
          >
            <Image
              src={src}
              alt={`Lamborghini ${idx + 1}`}
              width={800}
              height={600}
              className="w-full h-[250px] md:h-[350px] object-cover transition-transform duration-500 group-hover:scale-105"
            />

            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 opacity-100 transition-opacity duration-300 flex items-end p-4">
